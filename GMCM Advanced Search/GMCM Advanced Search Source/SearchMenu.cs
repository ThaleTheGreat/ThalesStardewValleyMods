...
internal sealed class SearchMenu : IClickableMenu
{
    ...
    private void InitializeLayout()
    {
        ...
        resultsRect = new Rectangle(innerX, resultsY, innerWidth, resultsHeight);

        int scrollbarX = xPositionOnScreen + width + 8;
        if (scrollbarX + ScrollbarWidth > viewportWidth - 8)
            scrollbarX = xPositionOnScreen + width - ScrollbarWidth - 8;
    ...
    {
        DrawOptionSeparator(b, searchRowRect.Bottom);
    ...
    private void DrawModTooltip(SpriteBatch b, string titleText, string description)
    {
        ...
        pos.Y += gap;
        b.Draw(Game1.fadeToBlackRect, new Rectangle(x + padding, (int)pos.Y - 3, boxWidth - padding * 2, 1), new Color(125, 92, 62, 90));
        pos.Y += 4;
    ...
    private void DrawOptionSeparator(SpriteBatch b, int y)
    {
        Rectangle line = new(resultsRect.X, y, resultsRect.Width, RowSeparatorHeight);
        b.Draw(Game1.fadeToBlackRect, line, new Color(125, 92, 62, 65));
    }
    ...
    private sealed class GmcmVerticalScrollbar
    {
        ...
        {
            int height = Math.Min(ThumbHeight, Math.Max(1, bounds.Height));
            int minY = bounds.Y;
