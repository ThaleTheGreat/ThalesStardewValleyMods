    ...
    public class InfuserMenu : MenuWithInventory
    {
        ...
        private bool Highlight(Item i)
        {
            ...
            else
            {
                ...
                {
                    if (i?.QualifiedItemId == "(O)74")
                        return true;
                    else if (mw.QualifiedItemId is "(W)62" or "(W)63" or "(W)64")
                        return (i is StardewValley.Object dgai && dgai.ItemId == ItemIds.SoulSapphire);
                }
                else if (mainSpot.item is Tool)
                    return i?.QualifiedItemId == "(O)74";
                else
        ...
        public override void receiveLeftClick(int x, int y, bool playSound = true)
        {
            ...
            else if ( secondarySpot.containsPoint( x, y ) && mainSpot.item != null )
            {
                ...
                if (mainSpot.item is MeleeWeapon mw)
                {
                    if (heldItem?.QualifiedItemId == "(O)74")
                        doIt = true;
                    else if (heldItem is StardewValley.Object dgai && dgai.ItemId == ItemIds.SoulSapphire && (mw.QualifiedItemId is "(W)62" or "(W)63" or "(W)64"))
                        doIt = true;
                }
                else if (mainSpot.item is Tool)
                {
        ...
        private Item DoCraft()
        {
            ...
            if ( secondarySpot.item?.QualifiedItemId == "(O)74" )
            {
                EnchantAgain(result);
                if (--secondarySpot.item.Stack <= 0)
                    secondarySpot.item = null;
            }
            else if (secondarySpot.item is StardewValley.Object dgai2 && dgai2.ItemId == ItemIds.SoulSapphire && result is MeleeWeapon mw && ( mw.QualifiedItemId is "(W)62" or "(W)63" or "(W)64" ) )
            {
                var oldResult = result;
                switch (mw.QualifiedItemId)
                {
                    case "(W)62":
            ...
            if ( secondarySpot.item is StardewValley.Object dgai && dgai.ItemId == ItemIds.PersistiumDust )
            {
            ...
            Game1.player.Items.ReduceId($"(O){ItemIds.StellarEssence}", 25);
