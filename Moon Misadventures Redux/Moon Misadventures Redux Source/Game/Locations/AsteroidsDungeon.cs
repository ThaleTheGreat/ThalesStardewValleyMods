...
using ThaleTheGreat.MoonMisadventures.Game.Locations.DungeonLevelGenerators;
...
{
    [XmlType( "Mods_ThaleTheGreat_MoonMisadventures_AsteroidsDungeon" )]
    public class AsteroidsDungeon : LunarLocation
    {
        ...
        private readonly NetBool bossDefeated = new();
        ...
        private Vector2 bossArenaCenter;
        private bool bossRewardChestWasPresent;
...
#pragma warning restore AvoidNetField
        ...
        protected override void initNetFields()
        {
            ...
            NetFields.AddField(bossDefeated, "bossDefeated");
        ...
        private void OnFixTeleporter( Point tile )
        {
            ...
            global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( this,  tx, ty - 2, 8, "Paths", null, pathsTs );
            setAnimatedMapTile( tx, ty - 2, a, 300, "Front", "z_moon-teleporters", null, false );
            setAnimatedMapTile( tx, ty - 1, b, 300, "Front", "z_moon-teleporters", null, false );
            Tile t = this.map.GetLayer("Buildings").Tiles[ tx, ty ];
            string actionStr = t.Properties[ "Action" ].ToString();
            setAnimatedMapTile( tx, ty, c, 300, "Buildings", "z_moon-teleporters", actionStr.Replace( "Offline", "" ), false );
        ...
        private void DoSwitch( Point arg )
        {
            ...
            global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( this,  lockX, lockY - 2, getTileIndexAt( lockX, lockY - 2, "Buildings" ) - 116, "Buildings", null, map.TileSheets.IndexOf( map.GetTileSheet( getTileSheetIDAt( lockX, lockY - 2, "Buildings" ) ) ) );
            global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( this,  lockX, lockY - 1, getTileIndexAt( lockX, lockY - 1, "Buildings" ) - 116, "Buildings", null, map.TileSheets.IndexOf( map.GetTileSheet( getTileSheetIDAt( lockX, lockY - 1, "Buildings" ) ) ) );
            string prop = doesTileHaveProperty( lockX, lockY, "Action", "Buildings" ).Replace( "LunarLock", "LunarDoor" );
            global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( this,  lockX, lockY - 0, getTileIndexAt( lockX, lockY - 0, "Buildings" ) - 116, "Buildings", prop, map.TileSheets.IndexOf( map.GetTileSheet( getTileSheetIDAt( lockX, lockY - 0, "Buildings" ) ) ) );

            //playSoundAt( "switch", tileLocation );

            global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( this,  tx, ty, getTileIndexAt( tx, ty, "Back" ) + 1, "Back", null, map.TileSheets.IndexOf( map.TileSheets.First( t => t.Id == getTileSheetIDAt( tx, ty, "Back" ) ) ) );
        ...
        protected override void resetLocalState()
        {
            ...
            if (level.Value == BossLevel && Game1.IsMasterGame)
            {
                EnsureBossEncounter();
        ...
        {
            if ( indexOfStone == ItemIds.MythiciteOreMinable )
            {
                Game1.createMultipleObjectDebris(
                    "(O)" + ItemIds.MythiciteOre,
                    x,
                    y,
                    r.Next( 2, 5 ),
                    who?.UniqueMultiplayerID ?? 0,
                    this
                );
                return true;
            }
        ...
        public override bool performAction( string actionStr, Farmer who, xTile.Dimensions.Location tileLocation )
        {
            ...
            {
                if (warpFromPrev == warpFromNext)
                {
                    if (!bossDefeated.Value)
                    {
                        Game1.drawObjectDialogue(I18n.Message_BossExitLocked());
                        return true;
                    }
        ...
        internal void SetBossArenaCenter(Vector2 tile)
        {
            bossArenaCenter = tile;
        }

        private void EnsureBossEncounter()
        {
            if (level.Value != BossLevel || bossArenaCenter == Vector2.Zero)
                return;

            if (Mod.instance.IsEyeOfCthulhuDefeated())
            {
                Mod.instance.MarkEyeOfCthulhuDefeated();
                bossDefeated.Value = true;
            }

            if (bossDefeated.Value)
            {
                EnsureBossRewardChest();
                return;
            }

            bool bossAlive = characters.OfType<EyeOfCthulhu>().Any(boss => boss.Health > 0);
            if (!bossAlive)
                characters.Add(new EyeOfCthulhu(bossArenaCenter * Game1.tileSize));
        }

        internal void CompleteBossEncounter()
        {
            if (!Game1.IsMasterGame || bossDefeated.Value)
                return;

            Mod.instance.MarkEyeOfCthulhuDefeated();
            bossDefeated.Value = true;
            foreach (ServantOfCthulhu servant in characters.OfType<ServantOfCthulhu>().ToList())
                characters.Remove(servant);

            EnsureBossRewardChest();
            localSound("questcomplete");
            Game1.addHUDMessage(new HUDMessage(I18n.Message_BossDefeated()));
        }

        private void EnsureBossRewardChest()
        {
            if (bossArenaCenter == Vector2.Zero || Mod.instance.IsEyeOfCthulhuRewardClaimed())
                return;

            if (netObjects.TryGetValue(bossArenaCenter, out StardewValley.Object existingObject))
            {
                if (existingObject is Chest)
                    bossRewardChestWasPresent = true;
                return;
            }

            Chest chest = new Chest(playerChest: false, bossArenaCenter);
            ...
            netObjects.Add(bossArenaCenter, chest);
            bossRewardChestWasPresent = true;
        }

        private void UpdateBossRewardState()
        {
            if (!bossDefeated.Value || Mod.instance.IsEyeOfCthulhuRewardClaimed() || !bossRewardChestWasPresent)
                return;

            if (!netObjects.TryGetValue(bossArenaCenter, out StardewValley.Object existingObject))
            {
                Mod.instance.MarkEyeOfCthulhuRewardClaimed();
                return;
            }

            if (existingObject is Chest chest && chest.Items.Count == 0)
            {
                netObjects.Remove(bossArenaCenter);
                Mod.instance.MarkEyeOfCthulhuRewardClaimed();
            }
        }
