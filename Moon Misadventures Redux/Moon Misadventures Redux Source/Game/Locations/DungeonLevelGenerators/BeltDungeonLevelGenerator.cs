        ...
        public override void Generate( AsteroidsDungeon location, ref Vector2 warpFromPrev, ref Vector2 warpFromNext )
        {
            ...
            for ( int ip = 1; ip < points.Count - 1; ++ip)
            {
                for ( int ib = 0; ib < beltWidth / 6; ++ib )
                {
                    ...
                    {
                        global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( location,  ( int ) tile.X, ( int ) tile.Y, 237, "Back", null, ts );
                    }
            ...
            foreach ( var tile in tiles.ToList() )
            {
                ...
                if ( u && ( lu ^ ru ) && ( l ^ r ) )
                {
                    if ( !lu )
                    {
                        global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( location,  tx - 1, ty - 1, 237, "Back", null, ts );
                    ...
                    if ( !l )
                    {
                        global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( location,  tx - 1, ty, 237, "Back", null, ts );
                ...
                if ( l && ( lu ^ ld ) && ( u ^ d ) )
                {
                    ...
                    if ( !u )
                    {
                        global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( location,  tx, ty - 1, 237, "Back", null, ts );
            ...
            Action< int, int, int > setBuildingsOrAlt = ( tx, ty, t ) =>
            {
                ...
                if ( ot == -1 )
                    global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( location,  tx, ty, t, "Buildings", null, ts );
                else
                {
                    global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( location,  tx, ty, ot, "Buildings1", null, ts );
                    global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( location,  tx, ty, t, "Buildings", null, ts );
                }
            };
                ...
                */
                ...
                if ( !d )
                {
                    ...
                    if ( u )
                    {
                        ...
                        if ( l && r )
                        {
                            global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( location,  tx, ty, 266, "Buildings", null, ts );
                            global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( location,  tx, ty + 1, 295, "Back", null, ts );
                            if ( !dd )
                                global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( location,  tx, ty + 2, 382, "Back", null, ts );
                        }
                        else if ( !l && r )
                        {
                            global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( location,  tx, ty, 265, "Buildings", null, ts );
                            global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( location,  tx, ty + 1, 294, "Buildings", null, ts );
                            if ( !dd )
                                global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( location,  tx, ty + 2, 381, "Back", null, ts );
                        }
                        else if ( l && !r )
                        {
                            global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( location,  tx, ty, 267, "Buildings", null, ts );
                            global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( location,  tx, ty + 1, 296, "Buildings", null, ts );
                            if ( !dd )
                                global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( location,  tx, ty + 2, 383, "Back", null, ts );
                        }
                        else // !l && !r
                        {
                            // U shape
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if ( l && r )
                        {
                            ...
                            if ( !lu && ru && ld && rd )
                                global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( location,  tx, ty, 210, "Back", null, ts );
                            else if ( lu && !ru && ld && rd )
                                global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( location,  tx, ty, 211, "Back", null, ts );
                            else if ( lu && ru && !ld && rd )
                                global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( location,  tx, ty, 239, "Back", null, ts );
                            else if ( lu && ru && ld && !rd )
                                global::ThaleTheGreat.MoonMisadventures.MapTileHelper.SetMapTile( location,  tx, ty, 240, "Back", null, ts );
                        }
                        else if ( !l && r )
                ...
                while ( pending.Count > 0 )
                {
                    ...
                    Action<Vector2> checkAdjacency = ( tv ) =>
                    {
                        if ( checkTiles.Contains( tv ) )
                        {
                            ...
                            {
                                // Check for breakable edges within weapon-swing reach.
