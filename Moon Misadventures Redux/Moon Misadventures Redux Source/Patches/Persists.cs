...
namespace ThaleTheGreat.MoonMisadventures.Patches
{
    ...
    public static class IClickableMenuDrawPersistsHoverTextPatch
    {
        ...
        {
            MethodInfo[] matches = typeof(IClickableMenu)
                .GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.DeclaredOnly)
                .Where(method => method.Name == nameof(IClickableMenu.drawHoverText))
                .Where(method =>
                {
            ...
            if (matches.Length != 1)
                throw new MissingMethodException(typeof(IClickableMenu).FullName, $"{nameof(IClickableMenu.drawHoverText)}(SpriteBatch, StringBuilder, ...)");
        ...
        {
            MethodInfo drawTooltip = AccessTools.Method(
                typeof(Item),
            ...
            MethodInfo replacement = AccessTools.Method(
                typeof(IClickableMenuDrawPersistsHoverTextPatch),
                nameof(DrawPersistsMessage)
            );

            List<CodeInstruction> result = new();
            int replacements = 0;

            foreach (CodeInstruction instruction in insns)
            {
                if (instruction.Calls(drawTooltip))
                {
            ...
            if (replacements != 1)
                throw new InvalidOperationException($"Expected one Item.drawTooltip call in {nameof(IClickableMenu.drawHoverText)}, found {replacements}.");
