...
public sealed class ModEntry : Mod
{
    ...
    private void OnGameLaunched(object? sender, GameLaunchedEventArgs e)
    {
        ...
        if (!Helper.ModRegistry.IsLoaded(WalletToolsId) || !Helper.ModRegistry.IsLoaded(TractorModId))
            return;

        WalletToolsApi = Helper.ModRegistry.GetApi<IWalletToolsApi>(WalletToolsId);
        if (WalletToolsApi is null)
        {
            Monitor.Log("Wallet Tools API was not available. Tractor compatibility patch was not applied.", LogLevel.Warn);
            return;
        }
        ...
        MethodInfo? prefix = AccessTools.Method(typeof(ModEntry), nameof(BeforeTractorUpdateAttachmentEffects));
    ...
    private void RegisterGmcm()
    {
        ...
        GmcmApi = Helper.ModRegistry.GetApi<IGenericModConfigMenuApi>(GmcmId);
        if (GmcmApi is null)
            return;
    ...
    {
        gmcm.AddTextOption(ModManifest, getValue, setValue, () => name, () => tooltip, allowedValues, fieldId);
    }
    ...
    private object? GetWalletToolsModEntry()
    {
        ...
        return AccessTools.Field(WalletToolsApi.GetType(), "Mod")?.GetValue(WalletToolsApi);
    }
