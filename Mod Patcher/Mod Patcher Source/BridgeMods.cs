...
internal sealed partial class ModEntry
{
    ...
    private readonly List<RegisteredBridgePatch> RegisteredBridgePatches = new();

    private void TryRegisterBridgeChange(IContentPack pack, AssetPatchChange change)
    {
        if (change.Bridge is null || !string.Equals(change.Bridge.Kind, "ReflectionProxyBridge", StringComparison.OrdinalIgnoreCase))
        {
            this.Monitor.Log($"Ignored BridgeMods change from {pack.Manifest.UniqueID}: Bridge.Kind must be ReflectionProxyBridge.", LogLevel.Warn);
            return;
        }

        RegisteredBridgePatch patch = new()
        {
            PackId = pack.Manifest.UniqueID,
            Name = string.IsNullOrWhiteSpace(change.Name) ? pack.Manifest.Name : change.Name.Trim(),
            RequireAnySource = NormalizeIds(change.RequireAnySource.Count > 0 ? change.RequireAnySource : change.Sources.Select(source => source.ModID)),
            RequireAnyTarget = NormalizeIds(change.RequireAnyTarget.Count > 0 ? change.RequireAnyTarget : change.Targets.Select(target => target.ModID)),
            Bridge = change.Bridge
        };

        if (patch.RequireAnySource.Count == 0 || patch.RequireAnyTarget.Count == 0)
        {
            this.Monitor.Log($"Ignored BridgeMods change from {pack.Manifest.UniqueID}: RequireAnySource and RequireAnyTarget are required.", LogLevel.Warn);
            return;
        }

        this.RegisteredBridgePatches.Add(patch);
        this.LogDebug($"Registered BridgeMods patch '{patch.Name}' from {pack.Manifest.UniqueID}.");
    }
    ...
    private void ApplyBridgeModPatches()
    {
        if (this.RegisteredBridgePatches.Count == 0)
            return;
        ...
        bool hasDeclaredPair = this.RegisteredBridgePatches.Any(patch =>
        ...
        bridgeHarmony = new Harmony($"{ModManifest.UniqueID}.BridgeMods");
